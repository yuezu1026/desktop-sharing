...
> 共 **39 个界面条目 ⇒ 42 张线框图**（多出的 3 张：W1-05「降级链」四态各出一张）。
...
graph TD
  ...
  D --> E[发起连接<br/>W1-02]
  E --> F{可否直连?}
  F -->|可| G[直连会话<br/>W1-03 显示「直连·不消耗免费时长」]
  F -->|否| H[中继会话<br/>W1-03 显示「中继·在消耗免费时长」]
  H --> I[降级链<br/>W1-05 四态]
  H --> J[转直连引导页<br/>W1-07]
  I --> K[仅查看遮罩<br/>W1-06]
  I --> L[终止前告知]
...
→ 若中继：常驻指示器显示「中继 · 本次剩余免费时长 xx 分钟」（W1-01/W1-04）
→ 首次连接前必须过隐私告知（W5-01）
```
...
## 8. 需要你拍板的 6 个问题（低保真评审通常卡在这里）

1. **「免费中继时长」还是「免费时长」？**（对外口径统一，影响 8 个屏的文案）
2. **首连提示用「Toast 3 秒」还是「会话窗口内常驻一行」？**（我建议后者：Toast 会消失，而主主张需要被反复看到）
3. **余量折叠态是否显示百分比？**（我建议只显示"约剩 xx 分钟"这类粗粒度，避免用户把注意力放在计数上）
4. **降级到「仅查看」时，被控端是否也显示"画面被限制"？**（我建议显示中性提示——否则被控端不理解为什么操作不动了）
5. **运营操作台调额度是否强制填写原因？**（我建议强制 + 超过阈值需第二人复核，审计必需）
6. **要不要把本稿的第 1 批（W1/W2）先做成可点击的 HTML 原型走一遍流程？**（成本很低，能提前发现流程断点）
