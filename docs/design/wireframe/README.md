...
| 层 | 判据 | 处理 | 数量 |
|---|---|---|---|
| **① UI = 已拍板决策的执行体** 🔴 | 没有这个界面，**已拍板的决策/合规项就被违反**（例：禁止静默降码率 ⇒ 必须有「码率已降」明示） | **P0 必做**，且**不参与 ROI 排序** | 41 |
...
> 📊 **当前值**：共 **59 个界面条目 ⇒ 65 张线框图**，层分布 **① 41 / ② 3 / ③ 15**，优先级 **P0 51 / P1 8**。
> （📊 记号 = 该句必须与 6 个线框页 HTML 实测一致，由 `node tools/wireframe-consistency.mjs` 硬门禁；历史轮次的数字请用 ⏱ 记号，见 §11。）
> 图比条目多 **6 张**，原因只有一个：<b>两处「一屏拆四态」</b>（1 条目 → 4 图）—— <b>W1-05「降级链」四态</b>（+3）与 <b>W4-10「支付结果」四态</b>（+3）。
> （W1-06 / W1-06b 是**两个独立条目**，各占一图；🔴 **W1-10 / W2-08 为 `[新增·D19]` 的音频两帧**，也是独立条目 9b / 15c；🔴 **W1-11 / W6-09 为 `[新增·D22]` 的全屏沉浸两帧**，是独立条目 9c / 44；🔴 **W3-07 ~ W3-10 为 `[新增·D23]` 的账号自助管理四帧**，是独立条目 21b / 21c / 21d / 21e；🔴 **W5-07 / W5-08 为 `[新增·D24]` 的运营台认证两帧**，是独立条目 33b / 33c；🔴 **W4-09 ~ W4-12 为 `[新增·D25]` 的支付链路四条目 / 七帧**，是独立条目 29b / 29c / 29d / 29e。）
...
| 29c | 🔴 **支付结果四态**（已开通 / 未完成 / 🔴 **确认中** / 订单已关闭）`[新增·D25]`<br /><span class="tiny">⚠️ 内容归属**支付链路组**（29b ~ 29e），只因此为补稿而编号排后；本条目**一屏拆四态**（4 图）</span> | ① | P0 | 商业化 §13.2**#11** · 账号 §4.6 | [W4-10a](w4-计费与个人中心.html#w4-10a) [b](w4-计费与个人中心.html#w4-10b) [c](w4-计费与个人中心.html#w4-10c) [d](w4-计费与个人中心.html#w4-10d) |
...
| 画布 | 标准尺寸：桌面 `830×500`、窗口 `760×440`、移动 `330×620`、面板 `430×470` / `430×300`。**单帧可因内容加高**，以帧内 inline `height` 为准 ⇒ 🔴 **逐帧台账见 §6.2**（44 帧加高 + 4 帧自由画布，机器对拍）。改稿后必须跑 `node tools/wireframe-selfcheck.mjs`（几何：裁切 / 横向溢出 / 锚点 / 原型 JS）与 `node tools/wireframe-consistency.mjs`（计数 / 画布台账 / 引用 / 被控端红线），🔴 **两个退出码都为 `0` 才能提交** |
...
> 门禁 `node tools/wireframe-consistency.mjs` 拿本表与 6 个线框页 HTML 对拍（`C5` 缺项 / `C5-b` 多余 / `C5-c` 高度不符 / `C5-d` 自由画布缺项）。
...
| W3 账号与设备管理 | W3-01 `464` · W3-02 `420` · W3-05 `440` · W3-06 `460` · W3-07 `560` · W3-08 `480` · W3-10 `470` |
| W4 计费与个人中心 | W4-01 `560` · W4-03 `520` · W4-05 `460` · W4-06 `460` · W4-07 `440` · W4-08 `400` · W4-09 `740` · W4-10a `420` · W4-10b `440` · W4-10c `560` · W4-10d `440` · W4-11 `520` · W4-12 `620` |
| W5 合规实名与运营台 | W5-01 `420` · W5-02 `460` · W5-03 `530` · W5-04 `420` · W5-05 `460` · W5-06 `700` · W5-07 `620` · W5-08 `480` |
| W6 移动端手持 | W6-02 `640` · W6-03 `600` · W6-04 `474` · W6-05 `500` · W6-07 `560` · W6-08 `560` |
| **自由画布**（非标准宽度 · W1-05 一屏拆四态） | W1-05a 392×236 · W1-05b 392×236 · W1-05c 392×250 · W1-05d 392×230 |

**合计 65 帧** = 44 帧加高（上表）+ 4 帧自由画布（上表末行）+ 17 帧标准默认（下列，`height` 省略即默认，**不登记在本表**）：

W1-01 · W1-02 · W1-10 · W1-11 · W2-01 · W2-02 · W2-03 · W2-04 · W2-06 · W3-03 · W3-04 · W3-09 · W4-02 · W4-04 · W6-01 · W6-06 · W6-09
...
| **⑤ 计数与编号不一致** | 页面自称条目数与 `index.html` / 本 README 不一致 | ⏱ 历史值：统一为 **43 个界面条目 ⇒ 46 张线框图**（2026-09-15 三轮补稿后为 **53 ⇒ 56**，见 §2 与 §11.7）；最新值见 §2 开头 |
...
> ⇒ ⏱ **历史值（`[D22]` 轮结束）**：全项目 **49 个界面条目 ⇒ 52 张线框图**，层分布 **① 33 / ② 3 / ③ 13**，优先级 **P0 41 / P1 8**（见 §1 / §2）。
...
> ⇒ ⏱ **历史值（`[D23]` 轮结束）**：全项目 **53 个界面条目 ⇒ 56 张线框图**，层分布 **① 36 / ② 3 / ③ 14**，优先级 **P0 45 / P1 8**（见 §1 / §2）。
...
> ⇒ ⏱ **历史值（`[D24]` 轮结束）**：该轮结束时为 **55 个界面条目 ⇒ 58 张线框图**（层分布 ① 37 / ② 3 / ③ 15，P0 47 / P1 8）。
...
> ⇒ 📊 **当前值**：全项目 **59 个界面条目 ⇒ 65 张线框图**，层分布 **① 41 / ② 3 / ③ 15**，优先级 **P0 51 / P1 8**（见 §1 / §2）。
...
### 12.1 两条门禁（在 `docs/design/wireframe/` 下执行）

```powershell
node tools/wireframe-selfcheck.mjs      # ① 几何：静默裁切 / 横向溢出 / 锚点 / 原型 JS
node tools/wireframe-consistency.mjs    # ② 一致性：计数 / 画布台账 / 引用 / 被控端红线
...
| # | 漂移类 | 检查项 | 触发条件 |
|---|---|---|---|
| ① | **计数漂移**（最贵，每轮 10+ 处手写同步点） | `C2` / `C2-marker` | 标 📊 的计数句与 6 个线框页 HTML 实测不一致 ⇒ **FAIL**；未标记的计数句 ⇒ WARN |
| ② | **几何漂移**（静默裁切 / 溢出 / 锚点失效） | `wireframe-selfcheck.mjs` | 子元素超出 `.screen` 边界 / `scrollWidth > clientWidth` / `href="#id"` 目标不存在 |
| ③ | 术语漂移 | 人工（本 README §6.1 术语表） | 暂无机器门禁，改稿时 grep 术语表 |
...
- **📊 当前值** = 该句必须与 6 个线框页 HTML 实测值一致 ⇒ 不一致 **直接 FAIL**（硬门禁）。
