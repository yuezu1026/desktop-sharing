...
## 1. 结论先行：这个项目一共有 **75 个需要 UI 的位置**

判定标准沿用 `商业化与计费设计` §13.1 的四类区分，本稿把它从"计费"扩展到全部端：

| 层 | 判据 | 处理 | 数量 |
|---|---|---|---|
| **① UI = 已拍板决策的执行体** 🔴 | 没有这个界面，**已拍板的决策/合规项就被违反**（例：禁止静默降码率 ⇒ 必须有「码率已降」明示） | **P0 必做**，且**不参与 ROI 排序** | 57 |
...
> 📊 **当前值**：共 **75 个界面条目 ⇒ 81 张线框图**，层分布 **① 57 / ② 3 / ③ 15**，优先级 **P0 67 / P1 8**。
> （📊 记号 = 该句必须与 7 个线框页 HTML 实测一致，由 `node tools/wireframe-consistency.mjs` 硬门禁；历史轮次的数字请用 ⏱ 记号，见 §11。）
> 图比条目多 **7 张**，原因只有一个：<b>两处「一屏拆多态」</b>—— <b>W1-05「降级链」四态</b>（4 图 = +3）与 <b>W4-10「支付结果」五态</b>（5 图 = +4，态 E 为 `[D27]` 补）。
...
| 画布 | 标准尺寸：桌面 `830×500`、窗口 `760×440`、移动 `330×620`、面板 `430×470` / `430×300`。**单帧可因内容加高**，以帧内 inline `height` 为准 ⇒ 🔴 **逐帧台账见 §6.2**（**46 帧加高 + 4 帧自由画布 + 31 帧标准**，机器对拍）。改稿后必须跑 `node tools/wireframe-selfcheck.mjs`（几何：裁切 / 横向溢出 / 锚点 / 原型 JS）与 `node tools/wireframe-consistency.mjs`（计数 / 画布台账 / 引用 / 被控端红线），🔴 **两个退出码都为 `0` 才能提交** |
...
**合计 81 帧** = 46 帧加高（上表）+ 4 帧自由画布（上表末行）+ 31 帧标准默认（下列，`height` 省略即默认，**不登记在本表**）：

W1-01 · W1-02 · W1-10 · W1-11 · W1-12 · W1-13 · W1-14 · W2-01 · W2-02 · W2-03 · W2-04 · W2-06 · W2-09 · W2-10 · W2-11 · W3-03 · W3-04 · W3-09 · W3-11 · W4-02 · W4-04 · W6-01 · W6-06 · W6-09 · W7-01 · W7-02 · W7-03 · W7-04 · W7-05 · W7-06 · W7-07
...
| 3 | 一键解绑全部设备 | 解除本账号下**全部**设备绑定（每台设备都要重新配对） | 登录密码 **且** 短信验证码 | [W3-06](w3-账号与设备管理.html#w3-06) 底部 | ⏳ 登记（确认页复用 W3-11 组件；触发按钮待补 `data-confirm`） |
| 4 | 关闭双重验证 | 账号安全等级回落（此后登录只靠密码 + 短信） | 登录密码 **且** 短信验证码 | [W3-03](w3-账号与设备管理.html#w3-03) | ⏳ 登记（该帧未出稿，见 §7 未出稿表） |
...
> ✅ **进展（第十轮 `[D27]`）**：**R19 / R20 / R21 / R22 / R24（控制端与账号侧）/ R25 / R26 / R30 / R32 已收口**；仍开着的是 R23 · R24 的 W5 侧 · R27 / R28 / R29 / R31 ⇒ 见 §11.12。
...
- ⏳ **仍未收**：R23（`.spec` 统一规格块）· **R24 的 W5 侧零表单错误态** · R27 / R28 / R29 / R31。
...
| ✅ | 跨帧 | **交叉引用帧号订正**：W3-11 台账与调用方把「一键解绑全部」`W3-04`→[W3-06](w3-账号与设备管理.html#w3-06)、「关闭双重验证」`W3-07`→[W3-03](w3-账号与设备管理.html#w3-03)（**引用 `#w#-##` 前必须先查该 id 的标题**，凭记忆必错） |
