...
## 1. 结论先行：这个项目一共有 **68 个需要 UI 的位置**

判定标准沿用 `商业化与计费设计` §13.1 的四类区分，本稿把它从"计费"扩展到全部端：

| 层 | 判据 | 处理 | 数量 |
|---|---|---|---|
| **① UI = 已拍板决策的执行体** 🔴 | 没有这个界面，**已拍板的决策/合规项就被违反**（例：禁止静默降码率 ⇒ 必须有「码率已降」明示） | **P0 必做**，且**不参与 ROI 排序** | 50 |
...
> 📊 **当前值**：共 **68 个界面条目 ⇒ 74 张线框图**，层分布 **① 50 / ② 3 / ③ 15**，优先级 **P0 60 / P1 8**。
...
> ⚠️ **15d–15k 归属说明**：`15d` / `15e` 内容属 **PC 被控端**（[W2](w2-被控端.html)）；`15f`–`15k` 内容属**安卓被控端**，落在 `[D26]` 补稿的**独立新页 [W7](w7-安卓被控端.html)**（本节标题原已含「Android 免 Root」，此前只有文字声明、无界面证据）。**九帧全部 P0。**
...
→ 只读分叉：仅观看（W6-03：🔴 链路指示器常驻 + 「请求控制」单向上转）
...
| 画布 | 标准尺寸：桌面 `830×500`、窗口 `760×440`、移动 `330×620`、面板 `430×470` / `430×300`。**单帧可因内容加高**，以帧内 inline `height` 为准 ⇒ 🔴 **逐帧台账见 §6.2**（44 帧加高 + 4 帧自由画布 + 26 帧标准，机器对拍）。改稿后必须跑 `node tools/wireframe-selfcheck.mjs`（几何：裁切 / 横向溢出 / 锚点 / 原型 JS）与 `node tools/wireframe-consistency.mjs`（计数 / 画布台账 / 引用 / 被控端红线），🔴 **两个退出码都为 `0` 才能提交** |
...
**合计 74 帧** = 44 帧加高（上表）+ 4 帧自由画布（上表末行）+ 26 帧标准默认（下列，`height` 省略即默认，**不登记在本表**）：

W1-01 · W1-02 · W1-10 · W1-11 · W1-12 · W2-01 · W2-02 · W2-03 · W2-04 · W2-06 · W2-09 · W2-10 · W3-03 · W3-04 · W3-09 · W4-02 · W4-04 · W6-01 · W6-06 · W6-09 · W7-01 · W7-02 · W7-03 · W7-04 · W7-05 · W7-06
...
> ℹ️ 纯审计轮，**不新增 `D##`**（下一个可用仍是 `D27`）；第七轮的编号归属见 `## 11` 顶部指针。
...
| **S2** | 🔴 **危险度 ↔ 按钮权重反向** | **越危险的动作，视觉权重越低** —— 全稿最严重的系统性缺陷，且**每一帧单独看都"合理"** | [W2-02](w2-被控端.html#w2-02)「拒绝」弱于「允许」；[W2-04](w2-被控端.html#w2-04)→[W2-07](w2-被控端.html#w2-07)「断开」由 solid 降为 ghost；[W3-09](w3-账号与设备管理.html#w3-09)「退出所有设备」= `.btn.sm` ghost **且无二次确认** |
...
| # | 位置 | 缺口 | 严重度 |
|---|---|---|---|
| R19 | 跨帧（见上 S2） | **危险度 ↔ 按钮权重反向**：破坏性 / 不可逆动作不得低于常规动作权重，且须二次确认 | 🔴 P0 |
| R20 | 跨帧（见上 S1） | **术语与状态多源**：同一动作 / 状态必须全稿一名一词，且**默认值只有一个源** | 🔴 P0 |
| R21 | [W2-02](w2-被控端.html#w2-02) 等高危帧 | **请求授权帧不显示控制方身份**：用户只看到「有设备请求控制」，看不到**是谁**（账号 / 设备名 / 上次是否连过）⇒ 无法做有意义的判断 | 🔴 P0 |
| R22 | [W2-08](w2-被控端.html#w2-08) | **麦克风范围自相矛盾**：同一帧内「包含麦克风收到的声音」与「本机麦克风不在此列（属 P1）」并存 ⇒ 必须删掉一处 | 🔴 P0 |
| R23 | 全稿（R16 扩面） | **规格块混 UI 实测 ≥ 15 帧**（登记时为 6 处）⇒ 高保真前须引入统一 `.spec` 样式并加门禁断言 | 🟠 P1 |
| R24 | [W1](w1-控制端-连接与额度.html) / [W3](w3-账号与设备管理.html) / [W5](w5-合规实名与运营台.html)（R8 深化） | **「连接失败 / 超时」无帧** + W3/W5 共 18 帧**零表单错误态**（格式错 / 被占用 / 验证码错 / 2FA 错） | 🔴 P0 |
| R25 | [W1](w1-控制端-连接与额度.html) ⇄ [W4](w4-计费与个人中心.html)（见上 S5） | **转化链双向断裂**：用尽帧须有购买入口；购买帧须有「其实还有三条免费路」 | 🔴 P0 |
| R26 | [W7-02](w7-安卓被控端.html#w7-02) + W7（缺帧） | **缺「收到连接请求」确认帧**（被控端最该有的一屏）；且 W7-02「可被连接」是纯文本 tag，**看起来像状态却点不动** | 🔴 P0 |
| R27 | [W7-06](w7-安卓被控端.html#w7-06) | **条件分支并列排版**：「机型不支持」与「支持」两组并列 ⇒ 读者会误读为同级能力，须改为互斥分支表达 | 🟠 P1 |
| R28 | [W6](w6-移动端手持.html) 全页（R14 姊妹项） | **无横屏帧**：流 8 的「下缘上滑 / 右缘左滑」在横屏下是另一套命中区，却没有任何横屏示意 | 🟠 P1 |
| R29 | [W5-06](w5-合规实名与运营台.html#w5-06) | **3 动作的权限块可合并为矩阵**：现为三份并列说明 ⇒ 高保真阶段会变成三套重复 UI | 🟡 P2 |
| R30 | [W3-09](w3-账号与设备管理.html#w3-09) | **「退出所有设备」权重过低且无二次确认**（`.btn.sm` ghost）⇒ 账号安全的"核弹按钮"长成了边角小字 | 🔴 P0 |
| R31 | [W2-09](w2-被控端.html#w2-09) | **托盘菜单与主界面总开关的状态同步未定义**：菜单里的「可被连接」是唯一态展示，主界面改了开关后托盘如何跟随（立即 / 下次刷新）无约定 | 🟠 P1 |
...
- **确认已登记、本轮不重复上报**：R8（三类系统状态）· R9（空态无帧）· R10（音频无事前开关）· R11（开关三处）· R12（记住我的选择无撤销）· R13（死入口）· R14（触控目标）· R16（规格块）· R17（缺"后端下发"标注）· R18（排序与声明不符）。
...
- **方案 C（未执行）**：升格为 `D27` 并补 6–8 帧 ——
  [W7](w7-安卓被控端.html) 收到连接请求 · [W2-05](w2-被控端.html#w2-05) 音频事前开关 ·
  [W1](w1-控制端-连接与额度.html) 连接失败 / 超时 · [W4-10c](w4-计费与个人中心.html#w4-10c) 尾态 ·
  [W3-06](w3-账号与设备管理.html#w3-06) / [W3-09](w3-账号与设备管理.html#w3-09) 交叉验证。
- **不补帧的收口路径**：把 R19（按钮权重）· R20（术语）落成 **§6 低保真规范**里的两条强制约定
  ⇒ 成本远低于补帧，且能被门禁断言（权重可查 class、术语可查词表）。
...
- 真值快照（调试用，只读，打印 74 帧全清单、每页帧数、图比条目差值、全部计数句）：`node tools/lib/wireframe-scan.mjs`
