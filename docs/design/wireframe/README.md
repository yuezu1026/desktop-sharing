...
| 15e | 🔴 **启动 / 初始化失败态**（🔴 **被控不可用 ≠ 产品不可用**；每个失败原因必须带**可点的下一步动作**，错误码只放「复制诊断信息」）`[新增·D26]` | ① | P0 | v2 §3.2 · §3.5 · 本 README §6.5.3（**R8 已收口**） | [W2-10](w2-被控端.html#w2-10) |
...
| 画布 | 标准尺寸：桌面 `830×500`、窗口 `760×440`、移动 `330×620`、面板 `430×470` / `430×300`。**单帧可因内容加高**，以帧内 inline `height` 为准 ⇒ 🔴 **逐帧台账见 §6.2**（**46 帧加高 + 5 帧自由画布 + 33 帧标准**，机器对拍）。改稿后必须跑 `node tools/wireframe-selfcheck.mjs`（几何：裁切 / 横向溢出 / 锚点 / 原型 JS）与 `node tools/wireframe-consistency.mjs`（计数 / 画布台账 / 引用 / 被控端红线），🔴 **两个退出码都为 `0` 才能提交** |
...
| 🆕 命中区 | 🔴 **触屏可点元素的「实际命中区」≥ 44×44 CSS px**（拇指热区下限；iOS HIG = 44pt、Material = 48dp，取较松的 44）。**只约束 `W6-*` / `W7-*` 等触屏帧**；桌面帧（`W1`/`W3`/`W4`/`W5`）下限 **24×24**（`.btn.sm` 单行高 20px ⇒ **必须**透明外扩）。⚠️ **视觉尺寸小 ≠ 可点区域小，扩命中区不改视觉**（否则线框变成假高保真）⇒ 数值与**点名清单**见 **§6.5.4**；真机 44px 实测归视觉阶段 |
...
| 交互态 | **视觉态已补**（`:hover` / `:focus-visible` / 禁用态，见 §6.5.1），但本稿的按钮仍是 `<span class="btn">`（伪按钮）⇒ 🔴 **键盘不可达**；语义化替换（`<button>` + Tab 顺序核查）仍属视觉阶段。⚠️ **推论（`R13`）**：因此本稿**不得出现"看起来能点、实际没画"的入口** —— 未出稿的功能只能**移除**或**显式标灰 + 标 P1**，二者必居其一 |
...
### 6.5 组件状态与组合矩阵（机器可读 · 2026-09-15 新增）

> 🔑 **为什么要有这一节**：§6.3 / §6.4 管的是**按钮的权重**（做错了能不能撤回 / 用户想脱身时找不找得到）。
> 但另有三类跨帧问题此前只散落在各帧注释里 —— **状态**（点下去会怎样）、**组合**（多个开关同时开怎么办）、**够不够得到**（手指 / 命中区）——
> 同一个问题在不同帧会给出不同答案。本节把它们写成**可判定规则**，并明确哪些**必须出图**、哪些**靠规则收口**
> （避免「组合爆炸」变成永远不做的借口）。

#### 6.5.1 交互态与语义化（3 类元素 × 3 态）

| 元素 | 默认 | `hover` / `:focus-visible` | 禁用态 |
|---|---|---|---|
| `.btn` | 白底 + `--ink2` 描边 | 底 `--fill2`（实心则 `#151515`）+ **2px `--ink` 外描边** | 点线描边 + `--ink3` 文字 + `--fill` 底 |
| `.item`（列表行） | 无底 | 底 `--fill`（整行可点区域才有反馈） | 同上 |
| `.input` | `--line` 描边 | `:focus-within` 描边转 `--ink2` | — |

- 🔴 **禁止用「变淡」表达不可用**：`.tiny` 只有 10.5px，再降透明度等于看不见（定色下限见 6.5.4）。
- ⚠️ **「`hover` 有样式」≠「键盘可达」**：本稿按钮多为 `<span>` ⇒ 只有 `tabindex="0"` 或真 `<button>` 才会进 Tab 序列。
  本节只落地**视觉态**；语义化替换 + 全稿 Tab 顺序核查 ⇒ **视觉阶段**（§11.4 A 类 #1）。
- 🔴 **可读灰定色（原 §11.4 缺口 #3 · 本轮收口）**：正文灰统一 `--ink3-text = #6f6f6f`
  （对 `#fff` = **5.03:1**、对 `--fill #f5f5f5` = **4.61:1**，两者均 ≥ **WCAG AA 4.5:1**）；
  `--ink3 = #8f8f8f`（≈3.4:1）**降级为仅非文字用途**（虚线 / 边框 / 图标描边），🔴 **不得**再用于正文文案。

#### 6.5.2 面板开关的组合矩阵（W2-08 三开关 ⇒ 2³ 的收口规则）

[W2-08](w2-被控端.html#w2-08) 的三个开关**互相独立、两两不互斥**，因此 **2³ 不必逐格出图** ——
矩阵退化为「**3 个独立布尔 + 2 条边界规则**」：

| 开关 | 管什么 | 通道方向 |
|---|---|---|
| 传出本机声音 | 被控端**播放出来的**声音能否上行 | 被控 → 控制（上行） |
| 对讲 | 控制端说话能否从被控端扬声器播出 | 控制 → 被控（下行） |
| 开始采音时播放提示音 | **提示音本身**是否播放（提示音不参与上行） | 本机 |

| # | 边界规则 | 判定 |
|---|---|---|
| ① | 「传出本机声音」= **关**时，提示音开关**无对象**（组合「关 / 开 / 开」合法但**静默**） | 不得因此弹任何提示、不得自动打开「传出本机声音」 |
| ② | 任一开关变化 ⇒ 控制端**只看到中性状态**（「对方已静音本机声音」），且**不得**改画质 / 时长 / 额度 | 同 §13.4 红线②⑥（原型红线断言会当场报警） |

- 🔴 **不需要补的**：8 格逐格出图（无互斥 ⇒ 不存在「格子」）。
  **需要补的**：三开关**各自的「不可用」态**（无音频设备 / 系统混音被禁用 ⇒ 中性说明，见 [W2-08](w2-被控端.html#w2-08) note③）。
- 🔴 **两个音频开关不得合并成一个「音频」开关**：麦克风 ≠ 系统混音（见 [W2-11](w2-被控端.html#w2-11)）。

#### 6.5.3 通用四态组件（加载 / 空 / 错误 / 离线 · 原 **R8**）

**规格层收口，不逐屏出图**（全稿核心动作几乎全是网络动作 ⇒ 逐屏出图 = 76 处 × 4 态 = 三百张）。
四态**结构固定为四件**，缺一即视为未设计完：

| 态 | 四件结构 | 硬约束 |
|---|---|---|
| `loading` 加载 | 骨架灰条 ×3 + 「正在…」 | 超过 **2s** 才显示；**不得**用模态遮罩（可取消的动作不能被挡住） |
| `empty` 空 | 图标 + 一句**事实** + **一个下一步动作** | **不得**出现购买 / 升级入口（[W1-01b](w1-控制端-连接与额度.html#w1-01b) 是参考实现） |
| `error` 错误 | 原因（人话）+ **可点的下一步** + 「复制诊断信息」 | 错误码**只放**在「复制诊断信息」里；🔴 **被控端**错误态一律**不得**出现金额 / 升级提示 |
| `offline` 离线 | 中性横幅（**非模态**，不拦人）+ 重连 | 同 §9.1 断点 7：对方先断开时不得模态拦人 |

- 🔴 **四态都必须有「下一步」**：写不出下一步的态 = 设计没想完（「请求失败」必须能重试或返回，不能只有一句话）。
- 已出帧的实例：控制端 [W1-02](w1-控制端-连接与额度.html#w1-02) / [W1-06b](w1-控制端-连接与额度.html#w1-06b)、被控端 [W2-10](w2-被控端.html#w2-10)、空态 [W1-01b](w1-控制端-连接与额度.html#w1-01b)；**其余屏复用本节规格**。
- ⚠️ **组件本身（一帧四态并排）仍归视觉阶段**：本节的收口物是「结构 + 硬约束」，不是组件帧。

#### 6.5.4 命中区下限（原 **R14**）

| 端 | 视觉尺寸 | **实际命中区**（含透明外扩） | 相邻可点元素间距 |
|---|---|---|---|
| 触屏（`W6-*` / `W7-*`） | 可小于下限 | 🔴 **≥ 44 × 44 px** | ≥ 8 px |
| 桌面（`W1`–`W5`） | 可与命中区相等 | **≥ 24 × 24 px** | ≥ 4 px |

- 🔴 **点名项（触屏帧内）**：[W6-02](w6-移动端手持.html#w6-02) 底部 4 键 · [W6-05](w6-移动端手持.html#w6-05) 两个模态按钮 · 各帧「断开」。
  视觉尺寸小 ≠ 可点区域小：**扩命中区不改视觉**。
- ⚠️ **线框层不改视觉尺寸**（改了就成假高保真）；本节只钉**数值 + 点名清单** ⇒ 实施与真机 44px 实测归视觉阶段。
...
> 🔴 **只有三个小节是活跃待办、留在正文**：`§11.4`（**待收 1 项 + 已收口 2 项 + 范围声明 1 项**）· `§11.5.2`（R8–R18 共 11 项）· `§11.11`（R19 起，第九轮跨帧审计）。
...
### 11.4 缺口与范围声明（2026-09-15 重分类）

> 🔑 **为什么要重分类**：本节此前把「**待做**」和「**有意不做**」混在一张 4 行表里 ⇒
> 前者会被后者稀释（连续六轮读起来都像「还剩 4 个坑」）。现按**能不能动手**拆成 A / B / C。

**A. 待收项（1 项 · 线框层已到此为止）**

| # | 项 | 剩余动作 |
|---|---|---|
| A1 | 伪按钮是 `<span>` ⇒ **不进 Tab 序列**（`:focus` 视觉态已于本轮补齐，见 §6.5.1） | ⏳ **视觉阶段**：改用 `<button>`（或 `tabindex="0"`）+ 全稿 Tab 顺序核查。🔴 **线框层不再改**（改了就不再是低保真） |

**B. 本轮已收口（2 项 · 均改为可判定规则，不靠出图）**

| # | 项 | 收口方式 |
|---|---|---|
| B1 | 面板三开关的状态覆盖矩阵（[W2-08](w2-被控端.html#w2-08) ⇒ 2³）未逐格出图 | ✅ **§6.5.2**：三开关**互相独立、两两不互斥** ⇒ 2³ 退化为「3 个独立布尔 + 2 条边界规则」，**8 格不必出图**；真正要补的是三开关**各自的不可用态** |
| B2 | 低对比度标注文字（`.tiny.muted`）仅供评审 | ✅ **§6.5.1**：定色 `--ink3-text = #6f6f6f`（对 `#fff` 5.03:1 / 对 `--fill` 4.61:1，均 ≥ **WCAG AA 4.5:1**）；`--ink3` 降级为**仅非文字**用途 |

**C. 范围声明（1 项 · 有据，不算缺口）**

| # | 项 | 为什么不收 |
|---|---|---|
| C1 | 退款（流 4）/ 注销（流 5）无原型 | 逆向流程且 **UI 无歧义**：二次确认复用 §6.3 同一个组件，静态帧已覆盖（[W4-06](w4-计费与个人中心.html#w4-06)「申请退款」+ 五条联动 / [W3-06](w3-账号与设备管理.html#w3-06)「提交注销」）；原型的用途是回答「**按键点下去之后下一步在哪儿**」（§9.2），逆向流程不存在该疑问 ⇒ **不入原型**。若高保真阶段需要演示逆向流程，再补流 4 / 5 |

> ℹ️ **与本表配套的两个新节点**：§6.5.3（通用四态组件，原 `R8`）· §6.5.4（命中区下限，原 `R14`）——
> 二者此前只在 §11.5.2 挂着 `⏳`，现均落到 §6 通行规范（收口物 = **规则**，不是帧）。
...
> **本节当前仍开放 = 0 项**（`R8` / `R14` 已于 2026-09-15 收口进 §6.5.3 / §6.5.4，见 §11.4 B 表），其余逐行均已标状态。

| # | 位置 | 缺口 | 建议 |
|---|---|---|---|
| R8 | 全稿（不逐帧） | **缺三类系统状态**：加载中 / 请求失败 / 服务端不可达。而核心动作几乎全是网络动作（连接、登录、支付、实名核验、导出、调额度）；**连接失败**（设备离线 / 密码错 / 网络不可达）尚无帧 | 出一套通用「加载 / 空 / 错误 + 重试」组件规格，不必逐屏出图 **✅ 已收口（2026-09-15）**：落成 **§6.5.3 通用四态组件**（`loading` / `empty` / `error` / `offline` 各钉**四件结构 + 硬约束**，如 empty 不得有购买入口、error 的错误码只放「复制诊断信息」、被控端错误态不得出现金额）；已出帧实例 [W1-02](w1-控制端-连接与额度.html#w1-02) / [W1-06b](w1-控制端-连接与额度.html#w1-06b) / [W2-10](w2-被控端.html#w2-10) / [W1-01b](w1-控制端-连接与额度.html#w1-01b)，其余屏复用规格；组件帧本身归视觉阶段 |
...
| R14 | W6-01/02/03/05/07/08 | **移动端触控目标过小**：`.btn.sm` min-height 20px（[W6-02](w6-移动端手持.html#w6-02) 底部 4 个按钮挤在 330px 内） | 移动端可点元素**命中区 ≥ 44px**（可扩命中区不改视觉），尤其「断开」与 W6-05 两个模态按钮 **✅ 已收口（2026-09-15）**：数值与点名清单落成 **§6.5.4**（触屏 **≥ 44×44** 实际命中区 / 桌面 **≥ 24×24**；点名 [W6-02](w6-移动端手持.html#w6-02) 底部 4 键 · [W6-05](w6-移动端手持.html#w6-05) 两个模态按钮 · 各帧「断开」；相邻间距 ≥ 8px）。🔴 **线框层不改视觉尺寸**（改了就成假高保真）⇒ 实施与**真机 44px 实测**归视觉阶段 |
...
| ⑭ | 🔴 **常驻层体积膨胀**（内容漂移之外的**第二种漂移**：它不报错、不自相矛盾，只是每一轮都多收费，直到冷启动成本从 4k 悄悄涨到 20k） | `C20` | `AGENTS.md` > **1800 tok** ⇒ **FAIL**；`.github/instructions/*.md` 单份 > **40 行** ⇒ **FAIL**（触发层必须保持「一条规则一行」）。机器生成的 `docs/OUTLINE.md` **不检**：体积由文档总量决定，拦它没意义（新鲜度归 `C11`） |
...
| `.prettierignore` | 忽略线框 `*.html`、`*.css`、`tools/**/*.mjs`、`*.ps1`、报告 JSON、`docs/**/*.md`、**根级 `AGENTS.md` + `.github/**/*.md`**（harness 也是「人写一次、机器校验」的源材料） |
...
| 层 | 文件 | 规模 | 何时读 |
|---|---|--:|---|
| **L0** | `AGENTS.md`（仓库根） | ~1.4k | 🔑 **冷启动必读**：铁律 / 门禁命令 / 文档地图 / 决策编号。**禁止往里面加啰嗦**（一臃肿就变成第二个 README，那正是它存在的理由）；只允许增补**工具入口 / 纪律**级别的 1~2 行。🔴 现由 **`C20` 硬门禁**兑底（> 1800 tok ⇒ FAIL） |
